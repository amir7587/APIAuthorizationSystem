...
public class UserService {
	...
	public int add(UserDTO user) {
		...
		try {
			...
			if (!isUserExist(user.getUsername(), conn)) {
				// util.encryptPassword(user.getPassword()
				...
				// Asssigning the role to each user
				...
				tokenId = util.executeUpdate(util.prepareStatement(query, conn, Integer.toString(userId), token), conn);
	...
	public JSONObject updateIngredient(JSONObject ingredientData) {
		...
		try {
			...
			int totalUnit = ingredientData.getInt("total_unit");
			int leftUnit = ingredientData.getInt("left_unit");
	...
	public JSONObject signin(UserDTO user) {
		...
		try {
			...
			// String password=util.encryptPassword(user.getPassword());
